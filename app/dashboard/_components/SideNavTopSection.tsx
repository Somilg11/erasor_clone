    ...
    <Popover>
      <PopoverTrigger>
        <div className="flex items-center gap-3 hover:bg-slate-200 p-3 rounded-lg cursor-pointer">
          ...
          <h2 className="flex gap-2 items-center font-bold text-[17px] text-nowrap">
            {activeTeam?.teamName}
            <ChevronDown />
          </h2>
        </div>
      ...
      <PopoverContent className="ml-7 p-4">
        <div>
            ...
            <h2 key={index} className={`p-2 font-semibold hover:bg-blue-500 hover:text-white rounded-lg cursor-pointer mb-1 ${activeTeam?._id==team._id&&'bg-blue-500 text-white'}`} onClick={()=>setActiveTeam(team)}>{team.teamName}</h2>
            ))}
        </div>
